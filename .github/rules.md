...
**Priority**: 🔴 **Triggers**: Version bumps, releases, changelog generation, semantic versioning

### Standard-Version Integration

- **Always Use Standard-Version**: Never manually edit version in package.json
- **Commit Convention Requirement**: All commits MUST follow conventional commits for automatic versioning
- **Changelog Generation**: Let standard-version generate CHANGELOG.md automatically
- **Tag Creation**: Standard-version creates tags automatically, never manual tags
- **Release Process**: Use standardized release workflow through .copilot/scripts/

### Version Bump Rules

- **Patch (0.0.x)**: `fix:` commits, documentation updates, minor refactoring
- **Minor (0.x.0)**: `feat:` commits, new features without breaking changes
- **Major (x.0.0)**: Any commit with `BREAKING CHANGE:` footer or `!` after type
- **Pre-release**: Use `--prerelease` flag for alpha/beta/rc versions

### Release Workflow

```bash
# Standard release (patch/minor/major determined by commits)
./.copilot/scripts/release.ps1

# Specific version type
./.copilot/scripts/release.ps1 -ReleaseAs patch
./.copilot/scripts/release.ps1 -ReleaseAs minor
./.copilot/scripts/release.ps1 -ReleaseAs major

# Pre-release
./.copilot/scripts/release.ps1 -Prerelease alpha
```

### Version Validation Gates

- **Pre-Release Check**: All tests must pass before version bump
- **Lint Validation**: Code must pass linting before release
- **Build Verification**: Project must build successfully
- **Conventional Commits**: All unreleased commits must follow conventional format
- **Clean Working Directory**: No uncommitted changes allowed during release

✅ **Right**: `npm run test && npm run lint && npm run build && npm run release`  
❌ **Wrong**: Manually editing package.json version  
❌ **Wrong**: Creating tags manually with `git tag`  
❌ **Wrong**: Releasing with uncommitted changes

## Cross-Platform Compatibility

**Priority**: 🔴 **Triggers**: Script execution, development setup, team onboarding

### Universal Script Usage

- **Always Use NPM Scripts**: Never call .ps1 or .sh files directly
- **Cross-Platform Commands**: All npm run commands work identically on Windows/Linux/macOS
- **Automatic OS Detection**: Let run-script.js handle OS-specific execution
- **No OS-Specific Instructions**: Write documentation assuming universal commands

### Script Execution Rules

- **Windows**: PowerShell scripts with UTF-8 encoding and execution policy bypass
- **Linux/macOS**: Bash scripts with automatic executable permissions
- **Universal**: Node.js runner handles all OS detection and execution
- **Consistent Interface**: Same commands, same flags, same behavior across platforms

### Team Collaboration

- **Documentation**: Always reference npm scripts, never OS-specific paths
- **Setup Instructions**: Universal commands that work on any development environment
- **CI/CD**: Use npm scripts in workflows for maximum portability
- **Onboarding**: New team members get identical experience regardless of OS

✅ **Right**: `npm run commit`, `npm run release`, `npm run commit:analyze`  
❌ **Wrong**: `./.copilot/scripts/smart-commit.ps1`, `./copilot/scripts/release.sh`  
❌ **Wrong**: OS-specific setup instructions
