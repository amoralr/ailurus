...
**Interface MockDocument** contiene los campos: id, slug, title, content (Markdown completo 100-1000+ líneas), category, subcategory, path (ruta jerárquica completa), status (published/draft/archived), createdAt, updatedAt, createdBy, excerpt (resumen corto).
...
**Model Document** debe incluir campos: id (autoincrement), slug (unique), title, content (Text), category, subcategory (opcional), path (ruta jerárquica completa), excerpt (resumen opcional), status (enum con default DRAFT), timestamps (createdAt, updatedAt), createdBy (default "anonymous").
...
**Interface FolderNode** con estructura recursiva: id, name, type (folder/file), icon (emoji opcional), path (ruta completa), order (para sorting), children (array recursivo de FolderNode), slug (solo para type='file').
...
- Agregar tabla `folders` en schema con campos: id (autoincrement), name, type (folder/file), icon (opcional), path, order, parentId (self-referential para jerarquía), slug (solo para files), relaciones parent/children (FolderHierarchy).
...
**CATEGORIES**: 4 categorías fijas con id, name, icon (emoji), order:

1. getting-started "Getting Started" 🚀 (order: 1) - 3 documentos
2. architecture "Architecture" 🏗️ (order: 2) - 4 documentos
3. api-reference "API Reference" 📚 (order: 3) - 5 documentos
4. guides "Guides" 📖 (order: 4) - 3 documentos

Distribución: Getting Started (instalacion, quick-start, configuracion), Architecture (arquitectura, frontend, backend, database), API Reference (overview, documents, search, upload, websocket), Guides (editor-guide, markdown-syntax, deployment), Sin categoría (4 overviews).

### **ACCIONES REQUERIDAS:**

✅ **1. Actualizar API_CONTRACTS.md:**

Agregar endpoint GET /docs?category=:category para filtrar documentos por categoría. Query parameter category acepta valores: getting-started, architecture, api-reference, guides. Response lista documentos con estructura completa.

✅ **2. Actualizar FRONTEND_ARCHITECTURE.md:**

Agregar sección Filtros por Categoría. Componente CategoryFilter.tsx permite filtrar por: 🚀 Getting Started, 🏗️ Architecture, 📚 API Reference, 📖 Guides. Cada categoría tiene: id único, nombre display, icono emoji, order para sorting.
...
**Ejemplos de paths reales**: "Equipo/Proyecto/Getting Started/Primeros Pasos/Guía de Instalación", "Equipo/Proyecto/Architecture/Arquitectura del Sistema", "Equipo/Proyecto/API Reference/API de Documentos", "Equipo/Proyecto/Guides/Guía del Editor", "Equipo/Información del Equipo", "Recursos/Información de Recursos".
...
Componente Breadcrumb.tsx en frontend/src/shared/components/layout/ que recibe path (string), divide por "/" en segmentos, renderiza navegación con separadores " > " entre segmentos.
...
**Estructura markdown real**: Título Principal (h1), Secciones (h2), Subsecciones (h3), contenido con markdown (bold, italic, inline code), code blocks con syntax highlighting (TypeScript y otros lenguajes), tablas con columnas, listas ordenadas y no ordenadas, diagramas Mermaid.
...
Asegurar soporte completo para: Syntax highlighting (typescript, bash, json, prisma, sql, mermaid), Tablas con alineación, Links internos relativos, Mermaid diagrams rendering, Copy button en code blocks, Headings con anchor links.

✅ **2. Documentar en FRONTEND_ARCHITECTURE.md:**

Markdown Features Soportadas: Headings (h1-h6) con anchor links, Bold/italic/strikethrough, Code blocks con syntax highlighting (10+ lenguajes), Inline code, Tablas, Listas (ordered/unordered/nested), Links externos e internos, Imágenes con lazy loading, Diagramas Mermaid, Blockquotes, Horizontal rules.
...
**Model Document**: id, slug (unique), title, content, status (DocumentStatus, default DRAFT), createdAt, updatedAt, createdBy (default "anonymous").

### **Schema Requerido (Por Mocks):**

**Model Document**: Agregar campos category (getting-started/architecture/api-reference/guides), subcategory (opcional), path (ruta completa jerárquica), excerpt (resumen max 200 chars). Índices en category y path.

**Model Folder**: id, name, type (folder/file), icon (opcional), path, order, parentId (self-referential), slug (solo para files, FK a documents.slug). Relaciones parent/children (FolderHierarchy). Índices en parentId y path.

**Model Category**: id (string: getting-started/architecture/etc), name (display name), icon (emoji
order Int
...
✅ **2. Crear migration:** npx prisma migrate dev --name add_categories_and_folders

✅ **3. Seed database con 19 documentos reales:**

En prisma/seed.ts importar MOCK_DOCUMENTS de mocks/documents.mock y FOLDERS de mocks/folders.mock. Main function debe: insertar categories con createMany(CATEGORIES), iterar MOCK_DOCUMENTS insertando con create, insertar folder tree recursivo con insertFolderTree(FOLDERS).
...
✅ **1. GET /docs?category=:category** - Obtener documentos de categoría específica (ej: GET /docs?category=architecture). Response: array con id, slug, title, category, excerpt, etc.

✅ **2. GET /folders** - Obtener árbol completo de carpetas. Response: FolderNode[] recursivo.

✅ **3. GET /folders/:path** - Obtener nodo específico por path (ej: GET /folders/Equipo/Proyecto/Getting%20Started). Response: FolderNode con children.

✅ **4. GET /categories** - Obtener lista de categorías. Response: array con id, name, icon, order, count.
...
⏳ **2. CategoryFilter.tsx** - Filtro por categoría con iconos. Props: categories, selected, onChange.

⏳ **3. Breadcrumb.astro** - Muestra path jerárquico. Props: path (string tipo "Equipo/Proyecto/...").
...
**Iconos Emoji en folders.mock.ts** - Constante ICONS con mappings: folder_users (👥), folder_project (📦), folder_rocket (🚀), folder_architecture (🏗️), folder_books (📚), folder_book_open (📖), folder_footprints (👣), folder_settings (⚙️).
