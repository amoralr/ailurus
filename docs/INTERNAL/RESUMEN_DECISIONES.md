...
- documents: id, slug, title, content, status (draft/published/archived), created_at, updated_at, created_by
- documents_fts: FTS5 para búsqueda full-text en title y content
- analytics_events: event_type (page_view/search_query), metadata, timestamp
- search_logs: query, results_count, searched_at (para búsquedas sin resultados)
...
**Seguridad básica**: Rate limiting con ThrottlerGuard (100 req/min), CORS habilitado para localhost:4321, validación de inputs con decoradores IsString y MaxLength(10000) en content.
